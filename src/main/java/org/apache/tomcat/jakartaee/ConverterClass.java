 ...
 */
public class ConverterClass implements Converter, ClassFileTransformer {
    ...
    private static final Logger logger = Logger.getLogger(ConverterClass.class.getCanonicalName());
    private static final ManagerString sm = ManagerString.getManager(ConverterClass.class);
    ...
    public String toString() {
        return ConverterClass.class.getCanonicalName() + '[' + profile.toString() + ']';
    }

    @Override
    public boolean accepts(String filename) {
        String extension = Util.getExtension(filename);
        boolean result = "class".equals(extension);
        if (MigrationCLI.VERBOSE && result) {
            logger.log(Level.INFO, "!!!   ConverterClass selected for filename:" + filename);
        }
        return result;
    }
    ...
    protected boolean convertInternal(String path, InputStream src, OutputStream dest, EESpecProfile profile, ClassLoader loader)
        ...
        if (logger.isLoggable(Level.FINE)) {
            if (converted) {
                ...
                logger.log(Level.FINE, sm.getString("classConverter.noConversion", path.replace('/', '.')));
            }
        }
