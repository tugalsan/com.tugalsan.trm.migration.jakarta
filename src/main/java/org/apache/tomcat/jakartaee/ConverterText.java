...
public class ConverterText implements Converter {
    ...
    private static final Logger logger = Logger.getLogger(ConverterText.class.getCanonicalName());
    private static final ManagerString sm = ManagerString.getManager(ConverterText.class);
     ...
     * Default constructor.
     */
    public ConverterText() {
    }

    @Override
    public boolean accepts(String filename) {
        String extension = Util.getExtension(filename);
        boolean result = supportedExtensions.contains(extension);
        if (MigrationCLI.VERBOSE && result) {
            logger.log(Level.INFO, "!!!   ConverterText selected for filename:" + filename);
        }
        return result;
    }
    ...
    public boolean convert(String path, InputStream src, OutputStream dest, EESpecProfile profile) throws IOException {
        ...
        if (converted) {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, sm.getString("classConverter.noConversion", path));
            }
        }
